...
variable "resource_tags" {
  type        = list(string)
  description = "Add user resource tags to the Elasticsearch instance to organize, track, and manage costs. [Learn more](https://cloud.ibm.com/docs/account?topic=account-tag&interface=ui#tag-types)."
  default     = []
  validation {
...
variable "access_tags" {
  type        = list(string)
  description = "Add access management tags to the Elasticsearch instance to control access. [Learn more](https://cloud.ibm.com/docs/account?topic=account-tag&interface=ui#create-access-console)."
  default     = []
  ...
  validation {
    condition = !(
      ...
      !(
        ...
        length([for credential in var.service_credential_names : credential.name if credential.role == "Administrator"]) > 0) ||
        var.admin_pass != null
      )
    )
...
variable "elser_model_type" {
  type        = string
  description = "Trained ELSER model to be used for Elastic's Natural Language Processing. Possible values: `.elser_model_1`, `.elser_model_2` and `.elser_model_2_linux-x86_64`. Applies only if also 'plan' is set to 'platinum'. [Learn more](https://www.elastic.co/guide/en/machine-learning/current/ml-nlp-elser.html)"
