...
| Name | Type |
|------|------|
| [elasticsearch_cluster_settings.es_cluster_settings](https://registry.terraform.io/providers/phillbaker/elasticsearch/latest/docs/resources/cluster_settings) | resource |
| [elasticsearch_index.es_index](https://registry.terraform.io/providers/phillbaker/elasticsearch/latest/docs/resources/index) | resource |
...
| <a name="input_action_auto_create_index"></a> [action\_auto\_create\_index](#input\_action\_auto\_create\_index) | This corresponds to an index pattern that triggers automatic index creation | `string` | `null` | no |
| <a name="input_add_cluster_configuration"></a> [add\_cluster\_configuration](#input\_add\_cluster\_configuration) | Set it to true if cluster configuration is to be added | `bool` | `false` | no |
...
| <a name="input_cluster_max_shards_per_node"></a> [cluster\_max\_shards\_per\_node](#input\_cluster\_max\_shards\_per\_node) | The maximum number of shards that can be assigned to a single node in a cluster. | `number` | `null` | no |
| <a name="input_create_index"></a> [create\_index](#input\_create\_index) | Set it to true if index is to be created | `bool` | `false` | no |
...
| <a name="input_force_destroy"></a> [force\_destroy](#input\_force\_destroy) | Whether an existing index with the same name should be destroyed before creating a new one. | `bool` | `true` | no |
| <a name="input_index_name"></a> [index\_name](#input\_index\_name) | The name of the index of the elasticsearch | `string` | `null` | no |
...
| <a name="input_number_of_replicas"></a> [number\_of\_replicas](#input\_number\_of\_replicas) | The total number of replicas of the primary shard in an index | `number` | `1` | no |
| <a name="input_number_of_shards"></a> [number\_of\_shards](#input\_number\_of\_shards) | The total number of shards the indexed data is to be divided into | `number` | `1` | no |
